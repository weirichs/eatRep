...
2. Use replication methods (Bootstrap, jackknife or "Balanced repeated replicates") for inference statistics [@RN393; @RN487; @RN603].
...
`eatRep` requires R version 4.4 or higher. The package is available from [CRAN](https://cran.r-project.org/web/packages/eatRep/index.html):
...
R packages `reshape2`, `tidyr` or `data.table` provide functions for reshaping. Additionally, `eatTools` provides the `wideToLong()` function for easy reshaping into the required long format. We illustrate the functionality with the help of the wide format data `data.timss3` from the `BIFIEsurvey` package: 
...
Exemplary data contains 4668 rows according to 4668 persons. The following variables should be considered for reshaping: 
...
``` r
reformulas::isNested(bt2010[,"idstud"], bt2010[,"country"])
```
...
``` r
reformulas::isNested(bt2010[,"idstud"], bt2010[,"domain"])
```
...
resReading |> dplyr::select(-dplyr::any_of(c("label1", "label2", "modus", "comparison", "id"))) |> 
     rmarkdown::paged_table(options = pgtab_options)
```

<div data-pagedtable="false">
  <script data-pagedtable-source type="application/json">
{"columns":[{"label":["depVar"],"name":[1],"type":["chr"],"align":["left"]},{"label":["parameter"],"name":[2],"type":["chr"],"align":["left"]},{"label":["country"],"name":[3],"type":["chr"],"align":["left"]},{"label":["kb"],"name":[4],"type":["chr"],"align":["left"]},{"label":["year"],"name":[5],"type":["chr"],"align":["left"]},{"label":["est"],"name":[6],"type":["dbl"],"align":["right"]},{"label":["p"],"name":[7],"type":["dbl"],"align":["right"]},{"label":["se"],"name":[8],"type":["dbl"],"align":["right"]}],"data":[{"1":"score","2":"mean","3":"countryA","4":"reading","5":"2010","6":"511.563","7":"0","8":"3.515"},{"1":"score","2":"mean","3":"countryB","4":"reading","5":"2010","6":"508.601","7":"0","8":"4.651"},{"1":"score","2":"mean","3":"countryC","4":"reading","5":"2010","6":"534.234","7":"0","8":"3.266"},{"1":"score","2":"Ncases","3":"countryA","4":"reading","5":"2010","6":"1034.000","7":"NaN","8":"NaN"},{"1":"score","2":"Ncases","3":"countryB","4":"reading","5":"2010","6":"959.000","7":"NaN","8":"NaN"},{"1":"score","2":"Ncases","3":"countryC","4":"reading","5":"2010","6":"1086.000","7":"NaN","8":"NaN"},{"1":"score","2":"sd","3":"countryA","4":"reading","5":"2010","6":"77.362","7":"0","8":"2.133"},{"1":"score","2":"sd","3":"countryB","4":"reading","5":"2010","6":"85.103","7":"0","8":"3.811"},{"1":"score","2":"sd","3":"countryC","4":"reading","5":"2010","6":"81.846","7":"0","8":"2.777"}],"options":{"columns":{"min":{},"max":[10]},"rows":{"min":[10],"max":[10]},"pages":{}}}
  </script>
</div>

The analysis above includes one grouping variable ("country") and one competence domain ("reading") without any group comparisons. The output contains mean, standard deviation, and sample size. We see that `countryC` has the highest mean and `countryB` the lowest. 
...
##  
##  analysis.number hierarchy.level dependent.var imp.var groups.divided.by group.differences.by
##                1               0         score     imp                                     NA
##                2               1         score     imp           country                   NA
## 
## Assume unnested structure with 3 imputations.
## Create 92 replicate weights according to JK2 procedure.
```
...
##                1               0         score     imp                                   <NA>
##                2               1         score     imp           country                 <NA>
##                3               1         score     imp               sex                  sex
##                4               2         score     imp     country + sex                  sex
## 
## Assume unnested structure with 3 imputations.
## Create 92 replicate weights according to JK2 procedure.
```
...
##  analysis.number hierarchy.level dependent.var imp.var groups.divided.by group.differences.by
##                1               0         score     imp                                   <NA>
##                2               1         score     imp           country              country
##                3               1         score     imp               sex                 <NA>
##                4               2         score     imp     country + sex              country
## 
## Assume unnested structure with 3 imputations.
## Create 92 replicate weights according to JK2 procedure.
```
...
## 2 analyse(s) overall according to: 'group.splits = 0 1'.
##  
##  analysis.number hierarchy.level dependent.var imp.var groups.divided.by group.differences.by                            adjust
##                1               0         score     imp                                     NA                             FALSE
##                2               1         score     imp           country                   NA 3 vars: 'sexnum', 'mignum', 'ses'
## 
## Assume unnested structure with 3 imputations.
## Create 92 replicate weights according to JK2 procedure.
```
...
``` r
res    <- report2(quan, add = list(domain = "reading"))[["plain"]]
```

## 5. Regression analyses (repGlm)

`repGlm` allows to estimate linear and logistic regression models. To date, trend analyses do not incorporate linking errors. The reporting function `report2()` optionally allows to print the results to the console (if `printGlm` is set to `TRUE`). In the first example, the regression of reading competence on sex, SES is estimated in each country separately. For a valid interpretation of interaction effects, SES variable is standardized within each imputation:
