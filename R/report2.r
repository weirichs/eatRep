report2 <- function(repFunOut, trendDiffs = FALSE, add=list(), exclude = c("NcasesValid", "var", "sampleSize"), printGlm = FALSE,
           ...
           if(nrow(plain3)>0) {
              if(!any(grepl("____",plain3[,"group"]))) {splitS <- "___"} else {splitS <- "____"}
              second <- eatTools::halveString(plain3[,"group"], splitS)         
              if("chiSquareTest" %in% plain3[,"parameter"]) {
                 third <- matrix(data=NA, ncol = 2, nrow=nrow(plain3))
                 stopifnot(length(unique(plainn[,attr(out, "allNam")[["group"]]])) ==2)
                 for ( j in 1:2) {third[,j] <- unique(plainn[,attr(out, "allNam")[["group"]]])[j]}
              } else {
           ...
           if(nrow(plain4)>0) {
              plain4 <- do.call("rbind", by(data = plain4, INDICES = plain4[,"group"], FUN = function (grp) {
                        ...
                        if(!any(grepl("____",spl1))) {splitS <- "___"} else {splitS <- "____"}
                        spl2   <- strsplit(spl1, splitS)
           ...
           rownames(plain) <- NULL
           ret   <- list(plain = plain[,-match("row", colnames(plain))], comparisons = compar, group=groups, estimate = estim)
...
idFocRefGrpGROUPDIFF <- function(string, out, plainn, add) {
           ...
           if(unique(string[,"parameter"]) == "chiSquareTest") {
              kontras <- 1
              names(kontras) <- attr(out, "allNam")[["group.differences.by"]]
              stopifnot(length(unique(plainn[,attr(out, "allNam")[["group"]]])) ==2)
              string[,attr(out, "allNam")[["group"]]] <- paste(unique(plainn[,attr(out, "allNam")[["group"]]]), collapse=".vs.")
           }
...
createLabel1 <- function(plain, out) {
           label1 <- apply(X=plain, MARGIN = 1, FUN = function (zeile){
                     if(zeile[["parameter"]] == "chiSquareTest") {return(zeile[["label2"]])}
